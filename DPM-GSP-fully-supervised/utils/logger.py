...
def set_log(config, is_train=True):
    ...
    exp_name = config.diffusion.method+'_'+config.model.model
...
def train_log(logger, config):
    logger.log(f'lr={config.train.lr} unweighted_MSE={config.train.unweighted_MSE} diffusion_steps={config.diffusion.step} schedule={config.train.lr_schedule} weight_decay={config.train.weight_decay} '
               f'cat={config.model.cat_mode} skip={config.model.skip}')
