...
def train_log(logger, config):
    logger.log(f'lr={config.train.lr} diffusion_steps={config.diffusion.step} temperature={config.diffusion.temp} schedule={config.train.lr_schedule} weight_decay={config.train.weight_decay} '
               f' skip={config.model.skip} load start={config.train.load_start}')
